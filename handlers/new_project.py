...
from core.deployer import finalize_deploy, ensure_project_venv
...
def _clear_pending_project_state(context: ContextTypes.DEFAULT_TYPE) -> None:
    np = context.user_data.pop("new_project", None)
    if np and np.get("_pat") is not None:
        np["_pat"] = ""
        np.pop("_pat", None)
...
async def state_code_upload(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    tmp_pid = f"pending_{uuid.uuid4().hex}"
    np["tmp_pid"] = tmp_pid
...
async def state_github_private_url(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    np = context.user_data.get("new_project", {})
    # BUG-037: pop the PAT and overwrite the source slot immediately so the
    # token spends the minimum possible time inside context.user_data.
    token = np.pop("_pat", "")
    np["_pat"] = ""
    np.pop("_pat", None)
...
async def state_python_version(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    # BUG-023 / BUG-024: merge pending source into the real project dir
    # BEFORE creating the venv.  os.replace() on a non-empty destination
    # directory fails on most filesystems, so we now use _merge_into().
    src_dir = project_path(tmp_pid)                          # tmp: projects/<tmp_pid>/
    dst_dir = project_path(real_pid, user_id, name)          # real: projects/<uid>/<name>/
    if os.path.isdir(src_dir):
        _merge_into(src_dir, dst_dir)
        try:
            os.rmdir(src_dir)
        except OSError:
            pass

    # Create the venv only after the user's source has landed in dst_dir.
    await ensure_project_venv(real_pid, user_id, name, ver)
...
def _merge_into(src: str, dst: str) -> None:
    """Recursively move every entry under *src* into *dst*.

    Files are overwritten; directories are merged so that pre-existing
    contents of *dst* (such as a previously-created .venv) survive.
    This is the BUG-023 fix — os.replace() on a non-empty directory
    raises OSError, while shutil.move + recursion handles the conflict.
    """
    import shutil
...
async def cancel_flow(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    _clear_pending_project_state(context)
...
async def timeout_flow(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    _clear_pending_project_state(context)
    await _edit_flow(context, "⌛ Session timed out. Please start again.")
    return ConversationHandler.END
    ...
    return ConversationHandler(
        ...
        states={
            ...
            ConversationHandler.TIMEOUT: [
                MessageHandler(filters.ALL, timeout_flow),
            ],
        },
        fallbacks=[
