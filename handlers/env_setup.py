...
async def env_upload_save(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    if (update.message.document.file_size or 0) > 64 * 1024:
        ...
        return ConversationHandler.END
    # BUG-041: previously the .env tmp file was created in the system temp
    # directory with default permissions (world-readable on many hosts).
    # We now (a) create the temp file under PROJECTS_DIR/<pid>/... when
    # possible, falling back to the system temp dir; (b) immediately chmod
    # it to 0o600 so only the bot's UID can read it during the brief
    # processing window.
...
@require_member
async def env_edit_pick(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    """Show the list of variables; user then taps one to enter the
    edit conversation (entry point envk_edit_<pid>_<key>).

    FIX for BUG-019: this is registered as a plain CallbackQueryHandler in
    main.py — it is NOT inside any ConversationHandler.  Returning
    ConversationHandler.END from a non-conversation handler is a no-op
    that misleads future readers, so we now return None.
    """
    q = update.callback_query
    await q.answer()
...
async def env_key_action(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    q = update.callback_query
    await q.answer()
    m = re.match(r"^envk_(edit|del)_([0-9a-f]{32})_(.+)$", q.data)
    if not m:
        return ConversationHandler.END
...
def build_env_handlers() -> ConversationHandler:
    # FIX for BUG-018: the global env_key_action handler registered in
    # main.py now matches ONLY ``envk_del_...``.  The ``envk_edit_...``
    # callback is exclusively an entry point of this ConversationHandler
    # so it can drive the ENV_EDIT_VALUE state without being handled twice.
    return ConversationHandler(
        entry_points=[
            CallbackQueryHandler(env_add_entry,    pattern=r"^envadd_[0-9a-f]{32}$"),
            CallbackQueryHandler(env_upload_entry, pattern=r"^envup_[0-9a-f]{32}$"),
            CallbackQueryHandler(env_key_action,   pattern=r"^envk_edit_[0-9a-f]{32}_.+$"),
        ],
        states={
