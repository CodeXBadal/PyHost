...
async def _get_owned_project(update: Update, pid: str):
    ...
    if proj is None:
        try:
            await update.callback_query.message.edit_text(NOT_FOUND)
        except Exception:
...
_ANA_RE = re.compile(r"^ana_([0-9a-f]{32})(?:_(24h|7d|30d))?$")
...
async def analytics_entry(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    await q.answer()
    # FIX for BUG-015: callback patterns are ana_<pid>, ana_<pid>_24h,
    # ana_<pid>_7d, ana_<pid>_30d.  A plain .replace("ana_", "") left the
    # "_24h"/"_7d"/"_30d" suffix on the pid string, breaking the DB lookup.
    m = _ANA_RE.match(q.data or "")
    if not m:
        return
    pid = m.group(1)
    range_key = m.group(2) or "7d"
