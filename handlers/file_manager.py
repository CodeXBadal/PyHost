...
async def fm_entry(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if proj is None:
        return
    entries = list_tree(pid, "", user_id=proj["user_id"], name=proj["name"])
...
async def fm_cd(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    q = update.callback_query
    await q.answer()
    m = re.match(r"^fmcd_([0-9a-f]{32})_(.*)$", q.data)
    if not m:
        return
    ...
    if proj is None:
        return
    entries = list_tree(pid, rel, user_id=proj["user_id"], name=proj["name"])
...
async def fm_get(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    m = re.match(r"^fmget_([0-9a-f]{32})_(.+)$", q.data)
    if not m:
        return
    pid, rel = m.group(1), m.group(2)
    proj = await _get_owned_project(update, pid)
    if proj is None:
        return
    path = get_file_path(pid, rel, user_id=proj["user_id"], name=proj["name"])
