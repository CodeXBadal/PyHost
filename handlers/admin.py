...
# ────────────────────────────────────────────────────────────
# states
ADM_BCAST_TYPE, ADM_BCAST_TEXT, ADM_BCAST_IMAGE, ADM_BCAST_CONFIRM = range(500, 504)
...
async def admin_panel_callback(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    # BUG-042: verify admin BEFORE acknowledging the callback so non-admins
    # cannot probe the existence of admin handlers via the generic loading spinner.
    q = update.callback_query
    if not _is_admin(update.effective_user.id):
        await q.answer(ADMIN_ONLY, show_alert=True)
        return
    await q.answer()
    await _render_panel(update)
...
async def adm_users(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    # BUG-042: admin check before q.answer() (info-disclosure fix)
    q = update.callback_query
    if not _is_admin(update.effective_user.id):
...
async def adm_stats(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    # BUG-042: admin check before q.answer()
    q = update.callback_query
    if not _is_admin(update.effective_user.id):
        await q.answer(ADMIN_ONLY, show_alert=True); return
    await q.answer()
...
def _broadcast_confirm_keyboard():
    return _markup([[
        _btn("✅ Send Now", "bcast_confirm", style="success"),
...
async def _broadcast_send(context, send_fn) -> Tuple[int, int]:
...
async def adm_broadcast_text(update, context):
    ...
    context.user_data["adm_broadcast"] = {
        "kind": "text",
        "text": txt,
        "confirm_after": time.monotonic() + 30,
    }
    await update.message.reply_text(
        "⚠️ Broadcast queued. Review it and confirm after 30 seconds.",
        parse_mode=ParseMode.MARKDOWN,
...
async def adm_broadcast_confirm(update, context):
    """Send the queued broadcast exactly once.

    BUG-022: double-clicking “Send Now” previously spawned duplicate broadcasts.
    We now atomically claim the payload before the slow send loop starts.
    """
    q = update.callback_query
    if context.user_data.get("adm_broadcast_sending"):
        await q.answer("Broadcast already in progress…", show_alert=True)
        return ADM_BCAST_CONFIRM

    payload = context.user_data.get("adm_broadcast")
    if not payload:
        await q.answer()
        await q.message.edit_text("Broadcast payload expired.")
        return ConversationHandler.END
    remaining = int(payload["confirm_after"] - time.monotonic())
    if remaining > 0:
        await q.answer(f"Wait {remaining}s before confirming.", show_alert=True)
        return ADM_BCAST_CONFIRM

    payload = context.user_data.pop("adm_broadcast", None)
    if payload is None:
        await q.answer("Broadcast already in progress…", show_alert=True)
        return ADM_BCAST_CONFIRM
    context.user_data["adm_broadcast_sending"] = True
    await q.answer("Sending...")

    if payload["kind"] == "text":
        captured_text = payload["text"]
        async def send(uid):
            await context.bot.send_message(uid, captured_text, parse_mode=ParseMode.MARKDOWN)
    else:
        captured_file_id = payload["file_id"]
        captured_caption = payload["caption"]
        async def send(uid):
            await context.bot.send_photo(uid, photo=captured_file_id, caption=captured_caption,
                                         parse_mode=ParseMode.MARKDOWN)

    try:
        sent, failed = await _broadcast_send(context, send)
    finally:
        context.user_data.pop("adm_broadcast_sending", None)
    await q.message.edit_text(f"📢 Sent: {sent} | Failed: {failed}")
    return ConversationHandler.END


async def adm_cancel(update, context):
    context.user_data.pop("adm_broadcast", None)
    context.user_data.pop("adm_broadcast_sending", None)
    if update.callback_query:
...
def build_admin_handlers():
    ...
    broadcast = ConversationHandler(
        ...
        states={
            ...
            ADM_BCAST_CONFIRM: [CallbackQueryHandler(adm_broadcast_confirm, pattern=r"^bcast_confirm$")],
        },
