...
async def _render_panel(update: Update, project_id: str, edit: bool = True) -> None:
    ...
    text = PROJECT_PANEL_MSG.format(
        ...
        restarts_today=(proj.get("restart_count_today") or 0),
...
async def project_panel_callback(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    m = re.match(r"^(panel|start|stop|restart)_([0-9a-f]{24,32})$", data)
    if not m:
        return
    ...
    if action == "start":
        ok, err = await process_manager.start_container(
            pid, proj["run_command"], env_map,
    ...
    if action == "restart":
        ...
        if ok:
            await update_project(pid, {
                "status": "running",
                "restart_count_today": (proj.get("restart_count_today") or 0) + 1,
            })
