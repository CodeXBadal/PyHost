"""
All inline keyboards — PTB 22.7 style.

━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
BUTTON COLOUR GUIDE  (PTB 22.7 / Bot API 9.4)
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
  'primary'  → 🔵 Blue   — navigate, view, refresh, settings
  'success'  → 🟢 Green  — start, confirm, add, create, install, upgrade
  'danger'   → 🔴 Red    — stop, delete, remove, ban, destructive cancel
  None       → ⚪ Default — neutral: back, info-only, non-destructive cancel

NOTE: Requires Telegram client released after Feb 9 2026.
      Older clients show default grey buttons (graceful fallback).
"""
...
def _btn(text: str,
    ...
    Args:
        style: 'primary' (blue), 'success' (green), 'danger' (red), or None.
               Requires PTB >= 22.7 and Telegram client >= Feb 9 2026.
    """
    if url is not None:
...
def force_join_keyboard(channel_username: str) -> InlineKeyboardMarkup:
    ...
    return _kb([
        [_btn("📢  Join Channel", url=f"https://t.me/{ch}", style="primary")],
        [_btn("✅  I've Joined — Check Access", "auth_check", style="success")],
    ])
...
def cancel_keyboard() -> InlineKeyboardMarkup:
    return _kb([[_btn("✖️  Cancel", "cancel_flow", style="danger")]])
...
def upload_type_keyboard() -> InlineKeyboardMarkup:
    return _kb([
        ...
        [_btn("✖️  Cancel",                "cancel_flow",   style="danger")],
    ])
...
def confirm_cancel_keyboard(confirm_data: str,
    ...
    return _kb([[
        ...
        _btn("✖️  Cancel",  cancel_data,  style="danger"),
    ]])
...
def back_to_panel_keyboard(project_id: str) -> InlineKeyboardMarkup:
    return _kb([[_btn("◀️  Back to Panel", f"panel_{project_id}", style="primary")]])
...
def my_projects_keyboard(projects: list, page: int, total_pages: int,
    ...
    for p in projects:
        ...
        # Only add a clickable button if project_id is valid (guaranteed by
        # list_projects migration, but guard defensively here too).
        if pid:
            rows.append([_btn(label, f"panel_{pid}", style="primary")])
        else:
            rows.append([_btn(f"⚠️  {name}  (loading…)", "noop")])
...
def error_report_keyboard(project_id: str) -> InlineKeyboardMarkup:
    return _kb([
        ...
        [_btn("◀️  Back to Panel", f"panel_{project_id}", style="primary")],
    ])
...
def env_menu_keyboard(project_id: str, has_vars: bool) -> InlineKeyboardMarkup:
    ...
    rows.append([_btn("◀️  Back to Panel", f"panel_{project_id}", style="primary")])
    return _kb(rows)
...
def env_keys_keyboard(project_id: str, keys: List[str],
    ...
    for k in keys:
        ...
        icon  = "🗑️" if action == "del" else "✏️"
        style = "danger" if action == "del" else "primary"
        rows.append([_btn(f"{icon}  {k}", cb, style=style)])
    rows.append([_btn("✖️  Cancel", f"env_{project_id}", style="danger")])
    return _kb(rows)
...
def install_deps_keyboard(project_id: str) -> InlineKeyboardMarkup:
    return _kb([
        ...
        [_btn("✖️  Cancel",      f"panel_{project_id}",  style="danger")],
    ])
...
def logs_keyboard(project_id: str) -> InlineKeyboardMarkup:
    ...
    return _kb([
        ...
        [_btn("◀️  Back to Panel",    f"panel_{pid}",     style="primary")],
    ])
...
def backup_list_keyboard(project_id: str, backups: list) -> InlineKeyboardMarkup:
    ...
    rows.append([_btn("◀️  Back", f"backup_{project_id}", style="primary")])
    return _kb(rows)
...
def scheduler_action_keyboard(project_id: str) -> InlineKeyboardMarkup:
    return _kb([
        ...
        [_btn("✖️  Cancel",              f"sched_{project_id}",             style="danger")],
    ])
...
def delete_confirm_keyboard(project_id: str) -> InlineKeyboardMarkup:
    return _kb([
        ...
        [_btn("✅  No, Keep It Safe",     f"panel_{project_id}",  style="success")],
    ])
...
# ── File manager ───────────────────────────────────────────
# BUG-016: Telegram limits callback_data to 64 bytes.
_CB_MAX_BYTES = 64


def _safe_cb(prefix: str, project_id: str, rel_path: str) -> Optional[str]:
    cb = f"{prefix}_{project_id}_{rel_path}"
    if len(cb.encode("utf-8")) <= _CB_MAX_BYTES:
        return cb
    return None
...
def file_manager_keyboard(project_id: str, entries: list,
    ...
    for e in entries[:25]:
        ...
        prefix = "fmcd" if e["is_dir"] else "fmget"
        cb     = _safe_cb(prefix, project_id, e["rel_path"])
        style  = None if e["is_dir"] else "primary"
        if cb is None:
            rows.append([_btn(f"{icon}  {e['name']}  ⚠️ path too deep", "noop")])
            continue
        rows.append([_btn(f"{icon}  {e['name']}", cb, style=style)])
    if cur_rel:
        parent    = "/".join(cur_rel.rstrip("/").split("/")[:-1])
        parent_cb = _safe_cb("fmcd", project_id, parent) or f"fmcd_{project_id}_"
        rows.append([_btn("⬆️  Up One Folder", parent_cb, style="primary")])
    rows.append([_btn("◀️  Back to Panel", f"panel_{project_id}", style="primary")])
    return _kb(rows)
...
def admin_broadcast_type_keyboard() -> InlineKeyboardMarkup:
    return _kb([
        ...
        [_btn("✖️  Cancel", "admin_panel",            style="danger")],
    ])
...
def admin_user_actions_keyboard(target_user_id: int) -> InlineKeyboardMarkup:
    ...
    return _kb([
        ...
        [_btn("◀️  Back", "adm_users", style="primary")],
    ])
...
def upgrade_keyboard() -> InlineKeyboardMarkup:
    return _kb([
        [_btn("💎  Get Premium Now", "upgrade_pay", style="success")],
        [_btn("🏠  Back to Menu", "main_menu")],
    ])
