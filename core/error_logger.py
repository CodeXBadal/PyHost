...
async def collect_traceback(project_id: str, lines: int = 200,
                            user_id: int | str | None = None,
                            proj_name: str | None = None) -> str:
    """Return the tail of the container log (best-effort).

    FIX for BUG-012: forward user_id + proj_name to the process manager so
    the new-layout log file (projects/<user_id>/<name>/.pyhost.log) is
    actually read.  Without these the resolver fell back to the legacy
    projects/<project_id>/ path which never exists for committed projects,
    making every automatic crash report contain an empty traceback.
    """
    try:
        return await docker_manager.get_logs(
            project_id, lines=lines, errors_only=False,
...
async def on_container_crashed(*, bot: Bot, project_id: str,
    ...
    traceback_text = await collect_traceback(
        project_id, user_id=user_id, proj_name=project_name,
    )
    report = build_report(
