...
class DockerManager:
    ...
    def _project_path(self, project_id: str,
        ...
        FIX for BUG-014: honour the new projects/<user_id>/<name>/ layout
        when user_id + name are given; otherwise fall back to the legacy
        projects/<project_id>/ path used for tmp/pending uploads.
        """
    ...
    async def create_container(self, project_id: str, python_version: str,
                               ...
                               user_id: int | str | None = None,
                               name: str | None = None) -> str:
        """FIX for BUG-004: accept user_id / name so this signature matches
        ProcessManager.create_container().  The call site in core/deployer.py
        always passes them now; older callers that omit them still work.
        """
        ...
        host_path = self._project_path(project_id, user_id, name)
        os.makedirs(host_path, exist_ok=True)

        # remove dead leftover container with same name if any
        await self._safe_remove(container_name)
    ...
    async def start_container(self, project_id: str, run_command: str,
        ...
        invalid_keys = [k for k in env if not _ENV_KEY_RE.fullmatch(k)]
        if invalid_keys:
            return False, f"invalid env var name: {invalid_keys[0]}"
